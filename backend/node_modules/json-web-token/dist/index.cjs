...
function sign(alg, key, input) {
  if (alg.type === "hmac") {
    return crypto.createHmac(alg.hash, key).update(input).digest("base64url");
  }
  return crypto.createSign(alg.hash).update(input).sign(key, "base64url");
}
function verify(alg, key, input, signature) {
  ...
  return crypto.createVerify(alg.hash).update(input).verify(key, signature, "base64url");
}
